...
if(isset($_POST["updateArticleForm"])){
    ...
    $subject  = $articleObj->conn->validate($_POST["postTitle"]);    
    $body     = $articleObj->conn->validate($_POST["postBody"]);
    $category = $articleObj->conn->validate($_POST["postCategory"]);
    ...
    if($articleObj->updateArticle($params, $id)){
        if(move_uploaded_file($image_tmp, $article_dir.$image)){
            unlink($article_dir.$old_image);
        }
        $messages[] = "you've successfully updated this article";
    }else {
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="card text-center p-2">
                        ...
                        <span class="label label-sm bg-<?php echo substr($single_post["article_category"], 0, 4); ?>"> <?php echo $single_post["article_category"]; ?> </span><br/>
                        <small class="text-muted"> Written On <?php echo $single_post["article_date"]; ?> By <?php echo $single_post["article_author"]; ?> </small>
                        <p class="card-text text-justify"><?php echo $single_post["article_body"]; ?></p>
