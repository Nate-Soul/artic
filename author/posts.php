...
$myId       = $_SESSION["user"]["admin_id"];
$articleObj = new Articles();
$myarticles = $articleObj->getArticlesByAuthor($myId);
$catObj     = new Category();
$page       = "posts";
$errors     = array();
$messages   = array();
...
if(isset($_POST["createNewArticle"])){
    ...
    $subject   = Helper::sanitizeString($_POST["title"], "string");
    $body      = $_POST["body"];
    $category  = Helper::sanitizeString($_POST["category"], "string");
    $author    = Helper::sanitizeString($_SESSION["user"]["username"], "string");
    $author_id = Helper::sanitizeString($myId, "int");
...
<section id="posts" class="py-4">
        <div class="container">
            <div class="row">
                ...
                $searchArticles = $articleObj->searchMyArticles($myId, $q);
                ?>
                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-footer">
                            <?php if(count($myarticles) > 0){ echo $pageObj->getPaging(); } ?>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"]); ?>" id="addpostForm" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" id="category" class="custom-select">
                                ...
                                <option value="<?php echo $catmenu["name"]; ?>"><?php echo ucfirst($catmenu["name"]); ?></option>
                                <?php } unset($category); ?>
                                </optgroup>
                            </select>
                        </div>
